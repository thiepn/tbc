...
P2C does **not** claim that an automated check can prove every theological or interpretive judgment in 5,799 questions. It repairs only defects with a strong, reviewable semantic basis. Distractor quality, ambiguity calibration, difficulty, redundancy, and feedback pedagogy remain separate later P2 concerns.
...
- `phase6.jesus.luke.cross.criminal` — asked for a person; options are promises concerning the criminal.
...
- `phase9.nt.1-timothy.2.context` — asked for a place; options explain the role of prayer for rulers.
...
P2C restores semantically aligned, non-destructive stems while leaving answers, option sets, difficulty, routing, and gameplay behavior unchanged.
...
P2C changes only the embedded question engine and only through the 15 approved field transformations across the 12 confirmed questions. An exact preservation audit reconstructs the expected candidate from the P2B production merge and rejects any unrelated monolith change.
