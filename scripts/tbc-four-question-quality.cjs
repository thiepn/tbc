...
function main(){
  ...
  assert.deepEqual(changed.slice().sort(),[...cases.map(x=>x.id),BATCH03_ID].sort(),'only the four prior corrections and the separately tested Batch 03 identity may change');
